...
# Fonction 2 : Génération des réponses
def generate_responses(query: str, relevant_dir: str, top_k: int = 3):
    ...
    for file_name in relevant_files:
        ...
        image_paths.append(f"file://{os.path.abspath(im_path)}")  # Chemin absolu pour chaque image

    print(f"Number of images passed: {len(image_paths)}")

    # Préparer le prompt avec la requête
    PROMPT = f"""
    Use the following pages to answer the query:
    {query}
    """

    # Préparer les messages avec plusieurs images
    messages = [
        {
            "role": "user",
            "content": [
