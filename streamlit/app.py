...
with st.sidebar:
    ...
    qwen_model_name = st.selectbox(
        "Select Qwen Model",
        ["vidore/colqwen2-base", "Qwen/Qwen2.5-VL-3B-Instruct"],
        index=0,
    )
...
def save_images(images, save_dir):
    ...
    for i, img in enumerate(images):
        ...
        image_paths.append(temp_image_path)  # Simplifier pour l'affichage dans Streamlit
    return image_paths
    ...
    if pdf_file is not None:
        ...
        if not st.session_state.models_loaded:
            ...
            with st.spinner("Loading response model..."):
                ...
                # Adapte le chargement selon le modèle de génération choisi
        ...
        with chat_container:
            for msg in st.session_state.conversation:
                if msg["role"] == "user":
                    st.markdown(
                        f"<div style='text-align: right; background-color:#DCF8C6; padding:10px; border-radius:10px; margin:5px 0;'>"
                        f"<strong>You:</strong> {msg['text']}</div>",
                        unsafe_allow_html=True,
                    )
                else:
                    st.markdown(
                        f"<div style='text-align: left; background-color:#F1F0F0; padding:10px; border-radius:10px; margin:5px 0;'>"
